...
const CurrentPage = ({ data: state }) => {
  ...
  useEffect(() => {
    if (pathName.slice(1, 6) === "pages") {
      dispatch(
        setDaynamicPage({ pageName: pathName.slice(7), section: "page" })
      );
    } else if (pathName.slice(1, 11) === "categories") {
      dispatch(
        setDaynamicPage({ pageName: pathName.slice(12), section: "category" })
      );
    } else if (pathName.slice(1, 12) === "collections") {
      dispatch(
        setDaynamicPage({ pageName: pathName.slice(13), section: "page" })
      );
    }
  }, [pathName, dispatch]);
